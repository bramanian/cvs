...
class TipsController extends \BaseController {
    ...
    if(Agent::isMobile())
	{
	$content=View::make("frontend.smarttips.mcontent",array("tips"=>$tips))->render();	
		
	}else{
		
		$content=View::make("frontend.smarttips.content",array("tips"=>$tips))->render();
	}
	
	 return View::make('layout.main',array("content"=>$content,"breadcrumbs"=>array("Home"=>"/","Smart Tips"=>"")));		
	...
	public function showKategori($kategori){
		...
		$tips=Tips::join("kategori","kategori.id","=","tips.id_kategori")
					...
					->Orwhere("kategori.nama","like",$kategori."%")
					->Orwhere("kategori.nama","like","%".$kategori)					
				   ->selectRaw("tips.id as id,
				                tips.photo as photo,
				                tips.judul as judul,
								...
								kategori.nama as namakategori, kategori.nama as nama_kategori,kategori.small_image, kategori.large_image")
								->orderBy("tips.created_at","desc")->paginate(11);
    $content=View::make("frontend.smarttips.content",array("tips"=>$tips,"kategori"=>$kategori))->render();
	 return View::make('layout.main',array("content"=>$content,"breadcrumbs"=>array("Home"=>"/","Smart Tips"=>"/tips",$kategori=>"")));		
   ...
   public function search($judul){
	...
	$tips=Tips::join("kategori","kategori.id","=","tips.id_kategori")
	                   ->where("tips.judul","like","%".ArtikelController::getJudul($judul)."%")
    ...
    $content=View::make("frontend.smarttips.all",array("tips"=>$tips))->render();
   	...
   	return View::make('layout.main',array("content"=>$content,"breadcrumbs"=>array("Home"=>"/","Smart Tips"=>"/tips",ArtikelController::getJudul($judul)=>"")));	   
   ...
   public function showTag($judul){
    ...
    $content=View::make("frontend.smarttips.all",array("tips"=>$tips,"breadcrumbs"=>array("Home"=>"/","Smart Tips"=>"/tips",ArtikelController::getJudul($judul)=>"")))->render();
   ...
   public static function getTag($id){
	...
	$tips=Tips::join("kategori","kategori.id","=","tips.id_kategori")->where("kategori.tipe","=","tips")->orderBy("created_at","desc")	->selectRaw("tips.id as id, tips.id_user as id_user ,
					  tips.judul as judul, tips.isi as isi, 
					  tips.photo as photo,tips.created_at as created_at, 
					  tips.updated_at as updated_at, tips.isi as isi")->limit(10)->get();	
	...
	$tips=Tips::join("kategori","kategori.id","=","tips.id_kategori")->where("kategori.tipe","=","tips")->orderBy("created_at","desc")
	->selectRaw("tips.id as id, tips.id_user as id_user ,
					  tips.judul as judul, tips.isi as isi, 
					  tips.photo as photo,tips.created_at as created_at, 
					  tips.updated_at as updated_at, tips.isi as isi")->limit(6)->get();	
